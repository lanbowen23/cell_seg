...
def single_data_from_images(x_dir, y_dir, image_names, batch_size, bit_depth, 
                            dim1, dim2, rescale_labels):
    ## Prepare image names
...
def random_sample_generator(x_dir, y_dir, image_names, batch_size, bit_depth, 
                            dim1, dim2, rescale_labels):
    """
    image_names: list contain image name
    bit_depth: 8
    rescale_labels: 8 bit rescale label from 255 to 0, because skimage save it in 255
    crop_size: 256
    """
    ...
    # label is rgb, so shape should be 3
    ref_img = skimage.io.imread(os.path.join(y_dir, image_names[0]))
    if len(ref_img.shape) == 2:
        gray = True
    else:
    ...
    while True:
        ...
        for i in range(batch_size):
            ...
            # get random crop
            high = x_big.shape[0] - dim1
            if high == 0:
                start_dim1, start_dim2 = 0, 0
            else:
...
### add a second channel of double dip image with original image input
def single_data_from_images_2(x_dir, x_dir_2, y_dir, image_names, batch_size, bit_depth, dim1, dim2, rescale_labels):
    ## Prepare image names
    x_image_names = [os.path.join(x_dir, f) for f in image_names]
    x_2_image_names = [os.path.join(x_dir_2, f) for f in image_names]
    y_image_names = [os.path.join(y_dir, f) for f in image_names]

    # x_image_names_combine = []
    # for pair in zip(x_image_names, x_2_image_names):
    #     x_image_names_combine.append(pair[0])
    #     x_image_names_combine.append(pair[1])

    ## Load all images in memory
    x = skimage.io.imread_collection(x_image_names).concatenate()
    x_2 = skimage.io.imread_collection(x_2_image_names).concatenate()
    y = skimage.io.imread_collection(y_image_names).concatenate()

    ## Crop the desired size
    x = x[:, 0:dim1, 0:dim2]
    x = x.reshape(-1, dim1, dim2, 1)
    x_2 = x_2[:, 0:dim1, 0:dim2]
    x_2 = x_2.reshape(-1, dim1, dim2, 1)
    x = np.stack((x, x_2), axis=3)  # stack to 2 channels
    x = np.squeeze(x)
    y = y[:, 0:dim1, 0:dim2, :]
...
def random_sample_generator_2(x_dir, x_dir_2, y_dir, image_names, batch_size, bit_depth, dim1, dim2, rescale_labels):
    """generator for adding rough label as another channel on input

    This function will output x as a 2 channel image

    x_dir_2: config_vars["rough_labels_dir"]
    image_names: list contain image name
    bit_depth: 8
    rescale_labels: True
    crop_size: 256
    ...
    while True:
        ...
        # buffers for a batch of data
        x = np.zeros((batch_size, dim1, dim2, 2))
        y = np.zeros((batch_size, dim1, dim2, y_channels))
        ...
        for i in range(batch_size):
            ...
            # open images
            x_big = skimage.io.imread(os.path.join(x_dir, image_names[img_index][-8:])) * rescale_factor
            x_big_2 = skimage.io.imread(os.path.join(x_dir_2, image_names[img_index][-8:])) * rescale_factor
            y_big = skimage.io.imread(os.path.join(y_dir, image_names[img_index])) * rescale_factor_labels
            ...
            patch_x = x_big[start_dim1:start_dim1 + dim1, start_dim2:start_dim2 + dim2]  # * rescale_factor
            patch_x_2 = x_big_2[start_dim1:start_dim1 + dim1, start_dim2:start_dim2 + dim2]  # * rescale_factor
            patch_y = y_big[start_dim1:start_dim1 + dim1, start_dim2:start_dim2 + dim2]  # * rescale_factor_labels
            ...
            if do_augmentation:
                ...
                if rand_flip == 0:
                    patch_x = np.flip(patch_x, 0)
                    patch_x_2 = np.flip(patch_x_2, 0)
                    patch_y = np.flip(patch_y, 0)

                # rotate
                for rotate_index in range(rand_rotate):
                    patch_x = np.rot90(patch_x)
                    patch_x_2 = np.rot90(patch_x_2)
                    patch_y = np.rot90(patch_y)
